...
Lo script ti dice l'IP. Apri quell'URL dal browser del telefono. Fine.
...
- I tasti media usano `playerctl` (per i player), `wpctl` (PipeWire) e `brightnessctl`. Se la luminosità non risponde è probabile tu sia su monitor esterno senza backlight controllabile via sysfs — in quel caso serve `ddcutil` (non incluso).
